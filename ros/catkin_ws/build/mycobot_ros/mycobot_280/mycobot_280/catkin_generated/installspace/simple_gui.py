...
class Window:
    def __init__(self, handle):
        ...
        x = (self.ws / 2) - 190
        y = (self.hs / 2) - 250
        self.win.geometry("700x500".format(x, y)) # I'm buggin out spongebob me BOOOY!
        # layout,布局
        self.set_layout()
        # input section,输入部分
        self.need_input()
        ...
        tk.Button(self.frmLT, text="Set Joint", width=5, command=self.get_joint_input).grid(
            row=6, column=1, sticky="w", padx=3, pady=2
        )

        # coordination settings button,coordination 设置按钮
        tk.Button(self.frmRT, text="Set Cord", width=5, command=self.get_coord_input).grid(
            row=6, column=1, sticky="w", padx=3, pady=2
        )

        # Gripper switch button,夹爪开关按钮
        tk.Button(self.frmLB, text="Grip (Open)", command=self.gripper_open, width=5).grid(
            row=1, column=0, sticky="w", padx=3, pady=50
        )
        tk.Button(self.frmLB, text="Grip (Close)", command=self.gripper_close, width=5).grid(
    ...
    def need_input(self):
        ...
        tk.Label(self.frmRT, text=" YAW" ).grid(row=3)
        tk.Label(self.frmRT, text=" PITCH ").grid(row=4)
        tk.Label(self.frmRT, text=" ROLL ").grid(row=5)
